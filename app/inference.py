...
=======================
Loads trained LSTM models and runs predictions for CO2, temperature, and humidity.
"""
...
def predict_with_lstm(df_recent: pd.DataFrame, target: str) -> dict:
    """Run LSTM prediction — returns all 24 hourly predictions."""
    ...
    if any(x is None for x in [model, scaler_X, scaler_y, feat_cols]):
        return {"error": f"Model or scaler not found for '{target}'. Run train.py first."}

    X = df_recent[feat_cols].values[-SEQUENCE_LENGTH:]
...
def predict_all(df_recent: pd.DataFrame,
    ...
    for target in TARGET_COLS:
        results[target] = predict_with_lstm(df_recent, target)
...
def daily_forecast(df_recent: pd.DataFrame,
    ...
    -------
    {
        ...
        "co2_ppm":          {"mean", "min", "max", "unit", "hourly": [...]},
        "temperature_c":    {"mean", "min", "max", "unit", "hourly": [...]},
        "humidity_percent": {"mean", "min", "max", "unit", "hourly": [...]},
    }
    """
